...
# Plot 1: Average Global Temperature
# Cleaning
AvgGloTemp = AvgGloTemp[1275:3239, ]
AvgGloTemp_yearly = AvgGloTemp %>% group_by(Year) %>% summarise(Avg = mean(Anomaly), .groups = 'drop')
...
GloTemp$Trend = ifelse(GloTemp$Difference <= 0, "Not increasing", "Increasing")
GloTemp$Difference = abs(GloTemp$Difference)

# Plotting
P1 = ggplot(data = GloTemp) +
  geom_point(aes(x = Year, y = Avg, size = Difference, col = Trend), alpha = I(4/5)) +
  geom_smooth(aes(x = Year, y = Avg), col = "black", se = F, lty = 2) +
  theme_hc() +
...
# Interactivity
Plot1 = ggplotly(P1) %>%
  layout(title = list(text = paste0("<sup>", "Measured in degrees Celsius; Raw data source: berkeleyearth.org/data/", "</sup>"))) %>%
...
Mean_Sea_Level = Mean_Sea_Level %>% group_by(Year) %>% summarise(Change = last(GMSL), .groups = 'drop')
...
P2 = ggplot(data = Mean_Sea_Level) +
  geom_bar(aes(x = Year, y = Cumulative_change), stat = "identity", fill = "steelblue2") +
  geom_line(aes(x = Year, y = Cumulative_change), col = "royalblue4") +
  geom_point(aes(x = Year, y = Cumulative_change), col = "black") +
  xlab("") +
  ylab("Change\n") +
  theme_minimal()
...
# Plot 3: CO2 Emissions by Continent
...
CO2.Emi.continent = CO2.Emi.tidy %>% group_by(year, continent) %>% summarise(total = sum(emission), .groups = 'drop')
...
# Plot 4: Methane and NO Emissions
...
Methane.Emi.tidy = Methane.Emi.tidy %>% group_by(year) %>% summarise(emission = sum(emission), .groups = 'drop')
...
NO_Emi.tidy = NO_Emi.tidy %>% group_by(year) %>% summarise(emission = sum(emission), .groups = 'drop')
...
NO_CH4_Year = data.frame(Year = X, Emission = Y, Gas = Z)
...
# Plotting
P5 = ggplot(data = ET_Final) +
  geom_bar(aes(x = Period, y = Number, fill = Category, group = Category), stat = "identity", alpha = I(3/5)) +
